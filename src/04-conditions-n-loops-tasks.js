...
function findFirstSingleChar(str) {
  ...
  const firstSingleChar = Array.from(str).find((char) => charFrequency.get(char) === 1);
...
function isCreditCardNumber(ccn) {
  ...
  const reversedCcn = ccnStr.split('').reverse().join('');
  let sum = 0;
...
function isBracketsBalanced(str) {
  ...
  const isOpeningBracket = (char) => brackets[char];
  const isClosingBracket = (char) => Object.values(brackets).includes(char);

  const areBracketsMatching = (opening, closing) => brackets[opening] === closing;
  ...
  return str.split('').every((char) => {
    if (isOpeningBracket(char)) {
      ...
      if (!areBracketsMatching(lastOpening, char)) {
        return false;
      }
    }
    return true;
...
function toNaryString(num, n) {
  ...
  return toNaryString(quotient, n) + remainder.toString();
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let j = 0; j < 3; j += 1) {
    if (position[0][j] === position[1][j] && position[1][j] === position[2][j]) {
      if (position[0][j] === 'X') return 'X';
      if (position[0][j] === '0') return '0';
    }
  }
  if (
